...
public class Post extends AggregateEvent<PostId> {
    ...
    public void addFavorites(FavoritesId favoritesId, Author author, Star star) {
        Objects.requireNonNull(favoritesId);
        Objects.requireNonNull(author);
        Objects.requireNonNull(star);
        appendChange(new FavoritesAdded(favoritesId.value(), author.value(), star.value())).apply();
    }
